...
void CZhongGaoState::EnterState()
{
	...
	m_pMainUI = UIManager::Instance()->loadUI("Zhonggao.ui");
	if( !m_pMainUI ){
...
void CZhongGaoState::OnUpdate(float dt)
{
	...
	if( IsCHTOnly() ){
		...
		else if( true )	//临时修改，不显示语言选择界面，直接直接用简体
		{
...
void CZhongGaoState::OnDestroy()
{
	...
	UIManager::Instance()->removeUI("Zhonggao.ui");
	//LoginBack::instance();
	m_pMainUI = NULL;
}
/*
void	OnSelectLanguageBtnUP( UI* ui, void* data )
{
	...
	GameScene::GetScene()->ChangeState(GAME_STATE_CUTSCENE);
	UIManager::Instance()->removeUI( "ChooseLanguage.ui" );
}
*/
void	CZhongGaoState::OpenSelectLanguageTypeUI()
{
	...
	UI * pSelectUI = UIManager::Instance()->loadUI( "ChooseLanguage.ui" );
	if( !pSelectUI )
	{
		//GameScene::GetScene()->ChangeState(GAME_STATE_CUTSCENE);
		return;
